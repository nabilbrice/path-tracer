module pixels_mod
  ...
  type :: pixel_type
     ...
     integer(i32), dimension(3) :: spectrum = [0_i32, 0_i32, 0_i32]
...
contains
  ...
  !> directly
  function spectrum(image, normalised_coord)
    integer(i32), dimension(3) :: spectrum
    type(pixel_type), intent(in) :: image(:,:)
    real(r64), dimension(2), intent(in) :: normalised_coord

    ! These constantly get recomputed with every call
    ...
    spectrum = image(image_width, image_height)%spectrum
